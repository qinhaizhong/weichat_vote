...
if($do == 'installed') {
	...
	if (!empty($modules)) {
		foreach ($modules as $mid => $module) {
			...
			$modules[$mid]['official'] = empty($module['issystem']) && (strexists($module['author'], '-7') || strexists($module['author'], '微擎团队'));
...
if($do == 'permission') {
	...
	if (empty($module)) {
		message('你访问的模块不存在. 或许你愿意去微擎云服务平台看看. ', 'http://www.haoid.cn/?keyword=' . $_GPC['title']);
	}
...
<manifest xmlns="http://www.haoid.cn" versionCode="{$versions}">
